...
def main(page: ft.Page):
    ...
    async def pick_directory():
        ...
        if sys.platform == "win32":
            ...
            page_update()
            ruta = await file_picker.get_directory_path()
            ...
            def _select_via_zenity():
                try:
                    result = subprocess.run(
                        ['zenity', '--file-selection', '--directory',
                         '--title=Seleccionar carpeta de destino',
                         f'--filename={carpeta[0]}/'],
                        capture_output=True, text=True
                    )
                    if result.returncode == 0 and result.stdout.strip():
                        ruta_sel = result.stdout.strip()
                        carpeta[0] = ruta_sel
                        lbl_carpeta.value = ruta_sel
                        page_update()
                except FileNotFoundError:
                    set_estado("Instala zenity: sudo apt install zenity", ERR)
                except Exception as e:
                    set_estado(f"Error: {e}", ERR)
            threading.Thread(target=_select_via_zenity, daemon=True).start()

    def on_pick_dir(e):
        asyncio.create_task(pick_directory())
